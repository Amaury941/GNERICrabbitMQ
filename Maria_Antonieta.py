...
def main():
    ...
    channel.queue_declare("Mensagens_Maria_Antonieta",durable=True,arguments=args)

    channel.queue_bind(exchange=ExchangeOfChoice, queue = 'Mensagens_Maria_Antonieta', routing_key=KeyOfChoice)

    def callback(ch, method, properties, body): print(" [x] %r recebido" % body) ## printando as mensagens recebidas

    channel.basic_consume(queue='Mensagens_Maria_Antonieta', on_message_callback=callback, auto_ack=True) ## consumindo a queue Bahia
